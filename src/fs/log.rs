...
impl Log {
    ...
    pub fn write_log(&self) {
        for i in 0..self.lh.n {
            let log = get_buffer_block(self.head + i + 1, self.dev.as_ref().unwrap().clone());
            let mut buffer_guard = log.write().unwrap();
            buffer_guard.sync();
        }
    }
...
struct LogManager (Mutex<Log>, Condvar);
...
impl LogManager {
    ...
    pub fn write(&mut self, buffer: &RwLockWriteGuard<BufferBlock>) {
        ...
        if n == log_guard.lh.n {
            ...
            log_guard.buffer_outstanding.push(get_buffer_block(buffer.id(), dev));
        }
    }
}

static mut LOG_MANAGER : Lazy<LogManager> = Lazy::new(|| {
    LogManager::new()
});

#[cfg(test)]
mod test {
    use std::{sync::Arc, fs::{OpenOptions, File}, io::Write, os::unix::prelude::FileExt, thread};

    use super::super::filedisk::FileDisk;
    use super::*;
    use crate::fs::{fs::BlockDevice, filedisk};
