...
fn read_block(file: &mut File, block_id: u32, buf: &mut [u8]) {
    file.read_exact_at(buf, (block_id * BLOCK_SIZE) as u64).expect("read failed!");
}
...
pub fn mkfs(path: PathBuf, size: u32) {
    ...
    info!("log header: 2");
    info!("log: 3 - {}", 2 + nlog - 1);
    info!("inode blocks: {} - {}", 2 + nlog, 2 + nlog + ninodeblocks - 1);
    info!("free bit map: {} - {}", 2 + nlog + ninodeblocks, 2 + nlog + ninodeblocks + nbitmap - 1);
    info!("data blocks: {} - {}", 2 + nlog + ninodeblocks + nbitmap, fs_size - 1);
...
fn iappend(file: &mut File, inum: u32, sb: &SuperBlock, data: &[u8], freeblock: &mut u32) {
    ...
    let mut dst_block;
    info!("iappend: inum: {}, size: {}, off: {}, n: {}", inum, dinode.size, off, n);
    while n > 0 {
        ...
        buf[(off % BLOCK_SIZE) as usize..(off % BLOCK_SIZE + bytes) as usize].copy_from_slice(&data_ptr[..bytes as usize]);
...
fn rinode(file: &mut File, sb: &SuperBlock, inum: u32) -> Dinode {
    let mut buf = [0; BLOCK_SIZE as usize];
    info!("rinode: read inode block at block {}", block_of_inode(inum, sb));
...
fn winode(file: &mut File, sb: &SuperBlock, inum: u32, dinode: Dinode) {
    ...
    let dinode_bytes = bincode::serialize(&dinode).unwrap();
    buf[..dinode_bytes.len()].copy_from_slice(&dinode_bytes);
    info!("winode: write inode block at block {}", block_of_inode(inum, sb));
